...
- Web版はプロローグ後に志望校を選ぶ。志望校ごとに偏差値、合格学力、人望/メンツ条件、毎月の受験圧が変わる。
- Web版は最初に `受験番長` / `優等生ギャル` の主人公を選ぶ。ギャルルートは別立ち絵、別導入、別リアクション、別エンディングCGを使う。
- Web版は高校1年春の入学式から3年冬の卒業式まで、全36ターンで進行する。
- Web版は季節・学校行事・テスト時期に応じてBGMを切り替える。季節BGMは `/mnt/c/Users/minou/juken-bancho/web/assets/audio/seasons/`。
- Web版はエンディングごとに一枚絵と専用BGMへ切り替える。BGMがOFFの場合は、エンディング到達時も自動再生しない。
- Web版のスマホ表示は、操作ボタン3列、音量1行、2列ステータスHUD、下部スクロール式ダイアログを基本にする。画面遷移時はダイアログ枠内スクロールを先頭へ戻す。
