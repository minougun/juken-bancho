...
- レスポンシブ検証は `npm run test:responsive` で実行する。390x844、360x800、844x390、1280x800で主人公選択、導入、志望校、週次選択、最初の季節イベント分岐、結末帳、回想帳、全画面一枚絵まで確認し、スクリーンショットを `/mnt/c/Users/minou/juken-bancho/test-results/responsive-matrix/` に出力する。BGM OFF のまま初期フローを進めても `.mp3` / `.ogg` を読まないことも検出する。
